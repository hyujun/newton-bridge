...
**Terminal**: ROS 로그 라인. **기본 비활성** — 켜려면 `STATUS_LOG_HZ=1.0` (또는 다른 양수 Hz). 활성 시 평소엔 INFO, STALL 일 땐 WARN 으로 자동 승격:
```
[INFO] [newton_bridge]: [newton_bridge] sim 12.34s | step 240Hz (rt 1.00) | cmd 100Hz | pub 100Hz | render 60Hz | state=RUNNING
```
켰는데도 안 보이면 `LOG_LEVEL=INFO` (기본) 인지, root logger handler 가 살아있는지 확인.
